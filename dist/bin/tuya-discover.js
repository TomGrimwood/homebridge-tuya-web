...
const loginControl_1 = require("../src/api/loginControl");
const customerApi_1 = require("../src/api/customerApi");
const response_1 = require("../src/api/response");
...
function printHelp() {
    process.stdout.write(`tuya-discover — find your Tuya device IDs and local_keys
...
async function main() {
    ...
    const userCode = (_a = args.userCode) !== null && _a !== void 0 ? _a : (await prompt("Smart Life User Code (Me → Settings → Account & Security → User Code): "));
    if (!userCode) {
    ...
    const lc = new loginControl_1.LoginControl();
    const { token, qrData } = await lc.generateQrCode(userCode);
    if (!args.json) {
        process.stderr.write("\nScan this QR code with the Smart Life app (Me → Scan QR):\n\n");
        process.stderr.write(await qrcode_1.default.toString(qrData, { type: "terminal", small: true }));
        process.stderr.write("If the QR is mangled, paste this URL into a browser instead:\n  https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=" +
    ...
    const api = new customerApi_1.CustomerApi(tokenInfo, "HA_3y9q4ak7g4ephrvke", userCode, login.endpoint);
    const homesResp = await api.get("/v1.0/m/life/users/homes");
    ...
    for (const home of homes) {
        ...
        const devsResp = await api.get("/v1.0/m/life/ha/home/devices", { homeId });
        ...
        for (const d of devs) {
            ...
            const category = (_b = d.category) !== null && _b !== void 0 ? _b : "";
            discovered.push({
                name: d.name,
                id: d.id,
                local_key: d.local_key,
                ip: (_c = d.ip) !== null && _c !== void 0 ? _c : "",
                category,
                product_id: (_d = d.product_id) !== null && _d !== void 0 ? _d : "",
                device_type: (_e = response_1.CATEGORY_TO_DEV_TYPE[category]) !== null && _e !== void 0 ? _e : "switch",
                online: (_f = d.online) !== null && _f !== void 0 ? _f : false,
                home: home.name,
            });
        }
    }
...
// Output rendering
// --------------------------------------------------------------------------
function emitOutput(devices, asJson) {
    if (asJson) {
    ...
    process.stdout.write("=".repeat(72) + "\n\n");
    process.stdout.write("Replace the IP addresses below with the LAN IPs from your router\n");
    process.stdout.write("(the cloud only reports your house's WAN IP, which won't work).\n\n");
    const snippet = {
    ...
    process.stdout.write("Paste this into your Homebridge config.json `platforms` array,\n" +
...
//# sourceMappingURL=tuya-discover.js.map